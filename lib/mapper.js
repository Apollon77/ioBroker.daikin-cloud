/**
 * Class for mapping device data to ioBroker objects
 */
class ObjectMapper {
    /**
     * Initialize the ObjectMapper with empty Map collections
     */
    constructor() {
    ...
    /**
     * Define role for an object based on its ID and properties
     *
    ...
    defineRole(id, obj) {
        ...
        if (obj.type === 'boolean') {
            ...
            if (id.endsWith('Temperature')) {
                role += '.temperature';
            } else if (id.endsWith('Humidity')) {
                // Not seen, but maybe exists
                role += '.humidity';
            } else if (id.includes('.consumptionData.electrical.') && obj.unit === 'kWh') {
                // Not seen, but maybe exists
    ...
    /**
     * Parse a single datapoint and create object configuration
     *
     * @param {string} stateName - The name of the state
     * @param {object} stateData - The state data configuration
     * @param {string} parentIdPrefix - The parent object ID prefix
     */
    parseDatapoint(stateName, stateData, parentIdPrefix) {
        const common = {};
        ...
        if (stateData.unit !== undefined) {
            ...
            (stateName.endsWith('Temperature') || parentIdPrefix.endsWith('Temperature')) &&
            typeof value === 'number'
        ) {
            common.unit = '°C';
        }
    ...
    /**
     * Get objects for a data structure, recursively processing nested objects
     *
     * @param {object} data - The data object to process
     * @param {string} idPrefix - The object ID prefix
     * @param {string} parentName - The parent object name
     * @param {string} thisName - The current object name
     * @param {object} nativeBase - The base native configuration
     */
    getObjectsForStructure(data, idPrefix, parentName, thisName, nativeBase) {
        ...
        if (thisName === 'meta' || data.value !== undefined || data.settable !== undefined || data.unit !== undefined) {
            // We found the lowest level
            ...
            if (!this.objects.has(`${idPrefix}.${thisName}`)) {
                const common = this.parseDatapoint(thisName, data, idPrefix);
            ...
            Object.keys(data).forEach(key => {
                ...
                if (data[key] !== null && typeof data[key] === 'object') {
                    addedObjects = addedObjects.concat(
                        this.getObjectsForStructure(data[key], idPrefix, thisName, key, native),
                    );
                } else {
    ...
    /**
     * Convert a value when reading from the device
     *
     * @param {string} id - The object ID
    ...
    convertValueRead(id, value, obj) {
        ...
        if (obj && obj.native && obj.native.values) {
            ...
            obj &&
            obj.native &&
            obj.native.convertInfo === 'enabled-object'
        ) {
            value = value.enabled;
    ...
    /**
     * Convert a value when writing to the device
     *
     * @param {string} id - The object ID
     * @param {any} value - The value to convert
     * @param {object} obj - The object configuration
    ...
    /**
     * Update values in the data structure
     *
