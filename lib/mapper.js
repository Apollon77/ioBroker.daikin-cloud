class ObjectMapper {
    ...
    defineRole(id, obj) {
        ...
        if (obj.type === 'boolean') {
            ...
            if (id.endsWith('Temperature')) {
                ...
                role += '.humidity';
            } else if (id.endsWith('.consumptionData.electrical') && obj.unit === 'kWh') { // Not seen, but maybe exists
    ...
    getObjectsForStructure(data, idPrefix, parentName, thisName, nativeBase) {
        ...
        if (thisName === 'meta' || data.value !== undefined || data.settable !== undefined || data.unit !== undefined) { // We found the lowest level
            ...
            if (!native.managementPoint) {
                native.managementPoint = thisName;
            } else if (!native.dataPoint) {
                native.dataPoint = thisName;
            } else if (!native.dataPointPath && originalThisName.startsWith('/')) {
